...
##
##  24 juin 2023 : le c ne sert plus pour détecter les catégories
##                 questions numériques à réponses multiples possible [format QCM]
##                 format des réponses numériques plus riche : tolérance absolue possible [±]
##
##  12 jan. 2024 : bilan des types de questions converties, en fin de conversion
##                 traitement des notes pour ôter les espaces, si en texte…
##
##  14 fév. 2024 : correction d'un message dans le résumé final
##
##  12 avr. 2024 : correction d'un accent dans un avertissement
##                 erreur en cas d'absence de bonne réponse dans une question
##
##  15 avr. 2024 : mémorisation de la data.frame en cours de conversion
##                  ⇒ permet de la récupérer pour explorer l'erreur…
##
##  23 avr. 2024 : type QCI : qcm à notation « internat »
##
##  15 mai  2024 : corrigé la correction des notes « textes » : fonctionne
##                    même si pas de colonne de notes fournie…
##
##  29 mai  2024 : corrigé l'affichage des réponses numériques
##                 corrigé la détection des QCM internats quand aucun type de QCM indiqué
## ——————————————————————————————————————————————————————————————————————
...
{
    assign( "df.convertie", list( "Brute" = d ), envir = SARP.Moodle.env )
    ...
    if ( !is.na( colonne.note ) ) {
        if ( is.character( d[ , colonne.note ] ) ) {
            d[ , colonne.note  ] <- preparer_texte( d[ , colonne.note  ],
                                                   oter.insecables = TRUE ) # Note : peut contenir VRAI/FAUX et donc pb d'espace...
        }
    }
    ...
    lst <- get( "df.convertie", envir = SARP.Moodle.env)
    lst$Preparee = d
    assign( "df.convertie", lst, envir = SARP.Moodle.env )
    ...
    ## On fait un bilan de la conversion réalisée...
    n.questions <- length( unique( d$Numero.interne ) )
    cat( sep = "", "\r\n",
         "Nombre de questions trait\u00e9es : ", n.questions )
    if ( n.categories == 0 ) {
        cat( sep = "", " (sans indication de cat\u00e9gorie)" )
    } else {
        cat( sep = "", " (" )
        if ( d$I.Categorie[ 1 ] == FALSE ) {
            idx.C1 <- which( d$I.Categorie )[ 1 ]
            n.vrac <- length( unique( d$Numero.interne[ 1:idx.C1 ] ) )
            n.autres <- n.questions - n.vrac
            cat( sep = "", "dont ", n.vrac, " sans indication,",
                 " ", if ( n.autres > 1 ) "les autres" else "la derni\u00e8re" )
        } else {
            n.autres <- n.questions
        }
        if ( n.categories > 1 ) {
            cat( sep = "", " r\u00e9partie", if ( n.autres > 1 ) "s", " en" )
        } else {
            cat( sep = "", " plac\u00e9e", if ( n.autres > 1 ) "s", " dans" )
        }
        cat( sep = "", 
             " ", n.categories, " cat\u00e9gorie", if ( n.categories > 1 ) "s",
             ")" )
    ...
    idx.l1 <- which( !duplicated( d$Numero.interne ) )
    print( table( d$Type.final__[ idx.l1 ] ) )
                                       ...
                                       categorie.base )
{
    ...
    reponse.n <- trouver_reponse.numerique( reponse, "convertir.question_simple" )
    if ( is.finite( reponse.n$Nombre ) ) {
    ...
    ## if ( is.finite( reponse.n ) ) {
        ...
        if ( is.na( precision ) ) {
            if ( all( ( reponse.n$Nombre - as.integer( reponse.n$Nombre ) ) == 0 ) ) {
                ## Réponse apparemment entière : pas d'indication sur le nombre de décimales
                precision <- NA
                reponse.n$Nombre <- as.integer( reponse.n$Nombre )
            } else {
        ...
        numerique.moodle( texte = question[ 1, colonne.texte ],
                          bonne.reponse = reponse.n$Nombre,
                          titre = question[ 1, colonne.titre ],
                                ...
                                precision = precision )
{
    ...
    if ( length( bonnes ) == 0 ) {
        lst$QCM$Bonnes    <- bonnes
        lst$QCM$Mauvaises <- mauvaises
        assign( "df.convertie", lst, envir = SARP.Moodle.env )
        erreur( 350, "convertir_question",
                "Aucune bonne r\u00e9ponse indiqu\u00e9e..." )
    }

    ## Réponses numériques multiples
    if ( type.question %in% c( "n", "num\u00e9rique", "numerique", "nombre" ) ) {
        cat( "  num\u00e9rique,", n.reponses, "r\u00e9ponses possibles\n" )

        if ( length( mauvaises ) > 0 ) {
            avertissement( 1009, "convertir_question",
                           "Attention, les mauvaises r\u00e9ponses num\u00e9riques",
                           " seront ignor\u00e9es" )
        }
        
        ## On construit les réponses
        reponses <- trouver_reponse.numerique( question[ bonnes, colonne.reponse ],
                                               "convertir_question" )
        if ( any( is.na( reponses$Nombre ) ) ) {
            erreur( 1018, "convertir_question",
                    "Question num\u00e9rique demand\u00e9e,",
                    " mais r\u00e9ponse non-num\u00e9rique [",
                    paste0( "\"", 
                            reponses$Nombre[ which( is.na( reponses$Nombre ) ) ],
                            "\"", collapse = " ; " ),
                    , "]" )
        }
        ...
        numerique.moodle( texte         = question[ 1     , colonne.texte ],
                          bonne.reponse = reponses$Nombre,
                          notes         = question[ bonnes, colonne.note ],
                          n.decimales   = reponses$Precision, 
                          tolerance     = reponses$Tolerance, tolerance.type = 2,
                          titre         = question[ 1     , colonne.titre ],
                          fichier.xml = fichier.xml,
                          commentaire.global = commentaire.global,
                          note.question = note.question, penalite = penalite.question,
    ...
    cat( "  QCM,", n.reponses, "r\u00e9ponses possibles",
         if ( all( !is.na( type.question ),
                   type.question == 'qci' ) ) " [notation internat]",
         "\n" )
    ...
    qcm.moodle( texte = question[ 1, colonne.texte ],
                ...
                note.question = note.question, penalite = penalite.question,
                temps = temps,
                internat = if ( is.na( type.question ) ) FALSE else ( type.question == 'qci' ) )

    return( if ( all( !is.na( type.question ),
                      type.question == 'qci' ) ) "QCM Internat" else "QCM ou QCU" )
}
