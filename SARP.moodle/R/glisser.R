...
##
##   24 juin 2023 : essais de distracteurs sur le glisser-déposer de texte
##                  préparé pour les tags de questions
##
##   15 avr. 2024 : réparé une fausse manip ayant fusionné legender_image et glisser_texte
## —————————————————————————————————————————————————————————————————
                                   ...
                                   fichier.xml = get( "fichier.xml", envir = SARP.Moodle.env ) )
{
    ...
    if ( ! is.null( distracteurs ) ) {
        if ( is.vector( distracteurs ) ) {
            distracteurs <- data.frame( "Textes" = distracteurs,
                                        "Groupe" = 1,
                                        "Infini" = FALSE )
        }

        for ( i in 1:nrow( distracteurs ) ) {
            cat( file = fichier.xml, sep = "",
                 "    <dragbox>\n" )
            coder.texte( distracteurs$Textes[ i ], fichier.xml = fichier.xml )
            cat( file = fichier.xml, sep = "",
                 "      <group>", distracteurs$Groupe[ i ], "</group>\n",
                 if ( distracteurs$Infini[ i ] ) "      <infinite/>\n",
                 "    </dragbox>\n" )            
        }
    }
